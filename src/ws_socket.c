...
static bool check_auth(struct lws *wsi, struct pss_tty *pss)
{
    ...
    {
        return lws_hdr_custom_copy(wsi,
                                   pss->user,
                                   sizeof(pss->user),
                                   server->auth_header,
...
int callback_tty(struct lws               *wsi,
                 ...
                 size_t                    len)
{
    ...
    switch (reason)
    {
        case LWS_CALLBACK_FILTER_PROTOCOL_CONNECTION:
            ...
            if (n <= 0)
                n = lws_hdr_copy(wsi, pss->path, sizeof(pss->path), WSI_TOKEN_HTTP_COLON_PATH);
#endif
        ...
        case LWS_CALLBACK_ESTABLISHED:
            ...
            if (server->url_arg)
            {
                while (lws_hdr_copy_fragment(wsi, buf, sizeof(buf), WSI_TOKEN_HTTP_URI_ARGS, n++) >
                       0)
                {
        ...
        case LWS_CALLBACK_RECEIVE:
            ...
            switch (command)
            {
                ...
                case RESIZE_TERMINAL:
                    if (pss->process == NULL)
                        break;
                    json_object_put(parse_window_size(pss->buffer + 1,
                                                      pss->len - 1,
                                                      &pss->process->columns,
                                                      &pss->process->rows));
