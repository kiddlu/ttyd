...
static bool conpty_setup(HPCON          *hnd,
                         COORD           size,
                         ...
                         char          **out_name)
{
    ...
    *out_name = strdup(buf);
    ...
    if (!UpdateProcThreadAttribute(si_ex->lpAttributeList,
                                   0,
...
failed:
    ...
    *out_name = NULL;
    ...
    if (in_pipe != INVALID_HANDLE_VALUE)
        CloseHandle(in_pipe);
...
int pty_spawn(pty_process *process, pty_read_cb read_cb, pty_exit_cb exit_cb)
{
    ...
    if (!CreateProcessW(
            NULL, cmdline, NULL, NULL, FALSE, flags, NULL, cwd, &process->si.StartupInfo, &pi))
    {
    ...
    if (!RegisterWaitForSingleObject(
            &process->wait, pi.hProcess, conpty_exit, process, INFINITE, WT_EXECUTEONLYONCE))
    {
