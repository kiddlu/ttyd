...
static int send_unauthorized(struct lws *wsi, unsigned int code, enum lws_token_indexes header)
{
    ...
    if (lws_add_http_header_status(wsi, code, &p, end) ||
        lws_add_http_header_by_token(
            wsi, header, (unsigned char *)"Basic realm=\"ttyd\"", 18, &p, end) ||
...
static int check_auth(struct lws *wsi, struct pss_http *pss)
{
    ...
    {
        if (lws_hdr_custom_length(wsi, server->auth_header, strlen(server->auth_header)) > 0)
            return AUTH_OK;
        return send_unauthorized(
            wsi, HTTP_STATUS_PROXY_AUTH_REQUIRED, WSI_TOKEN_HTTP_PROXY_AUTHENTICATE);
    }
    ...
    switch (reason)
    {
        case LWS_CALLBACK_HTTP:
            ...
            if (strcmp(pss->path, endpoints.token) == 0)
            {
                ...
                if (lws_add_http_header_status(wsi, HTTP_STATUS_OK, &p, end) ||
                    ...
                    lws_finalize_http_header(wsi, &p, end) ||
                    lws_write(
                        wsi, buffer + LWS_PRE, p - (buffer + LWS_PRE), LWS_WRITE_HTTP_HEADERS) < 0)
                    return 1;
        ...
        case LWS_CALLBACK_OPENSSL_PERFORM_CLIENT_CERT_VERIFICATION:
            if (!len || (SSL_get_verify_result((SSL *)in) != X509_V_OK))
            {
                ...
                lwsl_err(
                    "client certificate verification error: %s (%d), depth: %d\n", msg, err, depth);
                return 1;
            }
            break;
#endif
