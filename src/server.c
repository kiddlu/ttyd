...
extern int callback_http(struct lws               *wsi,
                         ...
                         void                     *in,
                         size_t                    len);
extern int callback_tty(struct lws               *wsi,
                        enum lws_callback_reasons reason,
                        void                     *user,
                        void                     *in,
                        size_t                    len);
...
static void print_config()
{
    ...
    if (server->exit_no_conn)
        lwsl_notice("  exit_no_conn: true\n");
    if (server->index != NULL)
        lwsl_notice("  custom index.html: %s\n", server->index);
    if (server->cwd != NULL)
        lwsl_notice("  working directory: %s\n", server->cwd);
    if (!server->writable)
        lwsl_warn("The --writable option is not set, will start in readonly mode\n");
}
...
int main(int argc, char **argv)
{
    ...
    info.options =
        LWS_SERVER_OPTION_LIBUV | LWS_SERVER_OPTION_VALIDATE_UTF8 | LWS_SERVER_OPTION_DISABLE_IPV6;
#ifndef LWS_WITHOUT_EXTENSIONS
    ...
    while ((c = getopt_long(start, argv, opt_string, options, NULL)) != -1)
    {
        switch (c)
        {
            ...
            case 'I':
                ...
                {
                    fprintf(stderr,
                            "Can not stat index.html: %s, error: %s\n",
                            server->index,
                            strerror(errno));
                    return -1;
                }
...
#undef sc
            ...
            case 't':
                ...
                for (; optind < start && *argv[optind] != '-'; optind++)
                {
                    ...
                    {
                        fprintf(
                            stderr, "ttyd: invalid client option: %s, format: key=value\n", optarg);
                        return -1;
                    }
                    ...
                    json_object_object_add(
                        client_prefs, key, obj != NULL ? obj : json_object_new_string(value));
                }
                break;
            default:
                print_help();
    ...
    if (ssl)
    {
        ...
        info.options |=
            LWS_SERVER_OPTION_ALLOW_NON_SSL_ON_SSL_PORT | LWS_SERVER_OPTION_REDIRECT_HTTP_TO_HTTPS;
    }
#endif
